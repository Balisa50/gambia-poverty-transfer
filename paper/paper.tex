...
post-hoc and are reported as such. Coverage holding here is a contrast rather
than a foregone conclusion: concurrent work measuring conformal coverage on an
African satellite product under spatial shift reports it falling to 65.3\%
against a 90\% target. We conclude that cross-border transfer of wealth models
is better behaved than caution about distribution shift implies, in a region of
neighbouring countries sharing a survey instrument, and that the residual risk
lies in interval shape and in per-country variability rather than in average
coverage.
\end{abstract}
...
(Figures~\ref{fig:map} and~\ref{fig:heat}). If true
...
and 95\% (Figure~\ref{fig:heat}, bottom row). CQR was within 1.4 standard errors at every level. One of six
...
prediction under geographic shift. Two qualifications
