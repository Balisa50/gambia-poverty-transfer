...
(Figures~\ref{fig:bycountry}, \ref{fig:map} and~\ref{fig:heat}). Nine of
eleven countries fall
...
\begin{figure}[t]
\centering
\includegraphics[width=0.74\textwidth]{../figures/fig1_coverage_by_country.pdf}
\caption{Out-of-country coverage of a nominal 90\% interval, by held-out
country, against the interval within which sampling variation alone would place
the estimate if that country's true coverage were exactly 0.90 (grey). Nine of
eleven countries fall outside their own sampling band, which is the dispersion
quantified in Table~\ref{tab:disp}. Open circles give the in-country figure for
the same country, which is both closer to nominal and far less variable.}
\label{fig:bycountry}
\end{figure}
